...
void loop() {
  ...
  */
  if(LDRavr > 1023){
  LDRavr = 0; //Pick the highest out of the three LDR readings //int((lLDRval+mLDRval+rLDRval)/3); //Read and average the LDR values
  } else { 
    LDRavr = LDRavr + 1;
  }
  updateMovingAverage();
  ...
  delay(5); //Set delay to 200 for the moving average to average across 1s of time (5 * 200 = 1000 ms)
}
